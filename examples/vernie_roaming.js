...
poweredUP.on('discover', async (vernie) => {
  // Wait to discover Vernie
  ...
  if (vernie instanceof PoweredUP.BoostMoveHub) {
    ...
    vernie.on('distance', async (port, distance) => {
      if (distance < 150 && mode === Modes.ROAMING) {
        // If we're roaming around and we detect an object in front of us, stop and scan

        mode = Modes.AVOIDING
        vernie.setMotorSpeed('AB', 0)
        await vernie.sleep(1000)

        const { leftDistance, forwardDistance, rightDistance } = await scan(
          vernie
        )
    ...
    setInterval(async () => {
      if (vernie.current > 20 && mode === Modes.ROAMING) {
        // If current spikes, we've likely stalled on something. Reverse and scan.
