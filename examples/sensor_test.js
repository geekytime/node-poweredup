...
poweredUP.on('discover', async (hub) => {
  ...
  hub.on('tilt', (device, { x, y, z }) => {
    console.log(
      `Tilt detected on port ${device.portName} (X: ${x}, Y: ${y}${
        z !== 'undefined' ? `, Z: ${z}` : ''
      })`
    )
  })
  ...
  hub.on('accel', (device, { x, y, z }) => {
    console.log(
      `Accelerometer detected on port ${device.portName} (X: ${x}, Y: ${y}, Z: ${z})`
    )
  })

  hub.on('distance', (device, { distance }) => {
    console.log(
      `Motion detected on port ${device.portName} (Distance: ${distance})`
    )
  })
  ...
  hub.on('rotate', (device, { degrees }) => {
    console.log(
      `Rotation detected on port ${device.portName} (Rotation: ${degrees})`
    )
  })
  ...
  hub.on('remoteButton', (device, { event }) => {
    console.log(
      `Remote control button press detected on port ${device.portName} (Event: ${event})`
    )
  })

  hub.on('attach', (device) => {
    console.log(
      `Device attached to port ${device.portName} (Device ID: ${device.type})`
    )
  })
